...
$(function() {
    ...
    function(e) {
        ...
        var name = $("#name").val();
        var stuId = $("#stuId").val();
        var url = "/yx/welcome/loadRegisterStateByStuIdAndName/" + stuId + "/" + name;
    ...
    function(e) {
        e.preventDefault();
        var id = $("#id").val();
        var url = "/yx/welcome/loadRegisterStateById/" + id;
        $.get(url,
        function(student) {
...
function dataDeal(student) {
    var tbody = $("#page-wrapper > div > div.col_1.box > div.col-md-12 > div.panel.panel-warning > div.panel-body.no-padding > table > tbody");
    ...
    if (student.studentStatus.register == 0) {

        var registerStatus = "未完成报到及注册";
        tr.append($("<td></td>").attr("id", student.stuId).html(registerStatus));
        tr.append($("<td></td>").attr("id", 'btnId' + student.stuId).html("<a type='button' class='btn-info btn-sm btn'  onclick='updateRegisterStatusByStuId(" + student.stuId + ")'>确认报到&注册</a>"));

    } else {

        var registerStatus = "已完成报到及注册";
        tr.append($("<td></td>").html(registerStatus));
        tr.append($("<td></td>").html('<a type="button" class="btn-info btn-sm btn" disabled=“disabled”>确认报到&注册</a>'));
...
function updateRegisterStatusByStuId(stuId) {
    var url = "/yx/welcome/updateRegisterStatusByStuId" + "/" + stuId;
    $.get(url,
    function(data) {
        $("#" + stuId).html("已完成报到及注册");
